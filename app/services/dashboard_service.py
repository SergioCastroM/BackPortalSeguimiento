...
def dashboard_global(db: Session, anio: int, trimestre: int) -> dict:
    ...
    # Heatmap: % de metas activas con al menos un registro de seguimiento en cada trimestre (nunca > 100 %)
    all_sec = db.query(Secretaria).order_by(Secretaria.id).all()
    total_por_secretaria: dict[int, int] = {s.id: 0 for s in all_sec}
    for sid, cnt in (
        db.query(Meta.secretaria_id, func.count(Meta.id))
        .filter(Meta.activo == True)
        .group_by(Meta.secretaria_id)
        .all()
    ):
        total_por_secretaria[sid] = int(cnt or 0)

    reg_por_sec_trim: dict[tuple[int, int], int] = {}
    for sid, trim, cnt in (
        db.query(
            Meta.secretaria_id,
            SeguimientoMeta.trimestre,
            func.count(func.distinct(SeguimientoMeta.meta_id)),
        )
        .select_from(SeguimientoMeta)
        .join(Meta, and_(SeguimientoMeta.meta_id == Meta.id, Meta.activo == True))
        .filter(SeguimientoMeta.anio == anio)
        .group_by(Meta.secretaria_id, SeguimientoMeta.trimestre)
        .all()
    ):
        reg_por_sec_trim[(int(sid), int(trim))] = int(cnt or 0)
    ...
    for s in all_sec:
        ...
        for t in (1, 2, 3, 4):
            if tot == 0:
                ...
                reg = reg_por_sec_trim.get((s.id, t), 0)
                cobertura = round(100.0 * reg / tot, 1)
            heatmap.append(
                {
                    "secretaria_id": s.id,
                    "secretaria_nombre": s.nombre,
